...
SELECT TOP 1 order_id, AVG(list_price * (1-discount)) AS avegare_product_price
FROM sale.order_item
GROUP BY order_id
ORDER BY avegare_product_price DESC;
...
-- Question 6 is more logical than question 7 due to sorting. When you sort values by brand_id, it makes values more meaningful to lead to the conclusion. Otherwise, when list_price is decending, brand_id always will change row by row so it is nonsense for me.
